...
function renderNotes(){ const filter=$('#noteFilter')?.value||'all',rows=state.notes.filter(n=>filter==='all'||n.status===filter); if($('#notesOpenCount'))$('#notesOpenCount').textContent=state.notes.filter(n=>n.status!=='Done').length; if($('#notesDoneCount'))$('#notesDoneCount').textContent=state.notes.filter(n=>n.status==='Done').length; const host=$('#notesList'); if(!host)return; host.innerHTML=rows.length?rows.map(n=>`<article class="panel note-card ${n.status==='Done'?'note-done':''}" data-note-id="${n.id}"><div class="note-card-top"><span class="status ${noteStatusClass(n.status)}">${esc(n.status)}</span><small>${new Intl.DateTimeFormat('en-IN',{day:'2-digit',month:'short',year:'numeric'}).format(new Date(n.updated_at))}</small></div><p>${esc(n.note).replace(/\n/g,'<br>')}</p><div class="note-card-actions"><button class="btn btn-secondary" data-edit-note="${n.id}">Edit</button><button class="btn btn-ghost danger-text" data-delete-note="${n.id}">Delete</button></div></article>`).join(''):'<section class="panel empty-state"><div class="empty-icon">✎</div><h2>No notes yet</h2><p class="muted">Use the box above to add your first note.</p></section>' }
...
$('#noteForm').addEventListener('submit',saveNote); $('#cancelNoteEditBtn').addEventListener('click',resetNoteForm); $('#noteFilter').addEventListener('change',renderNotes);
