...
const state = { user:null, project:null, membership:null, areas:[], activities:[], purchases:[], purchaseOptions:[], team:[], teamOwner:null, page:'dashboard', authMode:'signin', passwordRecovery:false }
...
async function loadProjectData(){
  ...
  const [{data:areas},{data:activities},{data:purchases}]=await Promise.all([
  ...
  state.areas=areas||[]; state.activities=activities||[]; state.purchases=purchases||[]
...
function renderAll(){ renderDashboard(); renderActivities(); renderPending(); renderPurchaseCompare(); renderTeam(); $('#pendingBadge').textContent=state.activities.filter(a=>a.pending_with_me&&a.status!=='Completed').length }
...
function renderPurchaseCompare(){
  ...
  host.innerHTML=items.map(i=>{
    ...
    const cards=[1,2,3].map(no=>{ const o=opts.find(x=>x.option_no===no); if(!o)return `<div class="compare-option empty-option"><span class="option-kicker">OPTION ${no}</span><strong>Not added</strong><small>Add another brand/shop if you want a wider comparison.</small></div>`
      const avg=ratingAvg(o), tags=[o.is_selected?'<span class="compare-tag chosen">Your choice</span>':'',min!==null&&Number(o.price)===min?'<span class="compare-tag">Lowest price</span>':'',best&&avg===best?'<span class="compare-tag">Best rated</span>':''].filter(Boolean).join('')
      return `<div class="compare-option ${o.is_selected?'selected-option':''}"><div class="option-top"><span class="option-kicker">OPTION ${no}</span><div class="compare-tags">${tags}</div></div><h3>${esc(o.brand||'Unnamed option')}</h3><div class="model-line">${esc(o.model||'')}</div><div class="compare-price">${money(o.price)}</div><div class="source-line">${esc(o.source_name||'Source not entered')}${o.source_type?` · ${esc(o.source_type)}`:''}</div><div class="rating-grid"><span>Service <b>${stars(o.service_rating)}</b></span><span>Quality <b>${stars(o.quality_rating)}</b></span><span>Durability <b>${stars(o.durability_rating)}</b></span><span>Value <b>${stars(o.value_rating)}</b></span></div><div class="compare-meta"><span><small>Warranty</small><strong>${esc(o.warranty||'—')}</strong></span><span><small>Overall</small><strong>${avg?avg.toFixed(1)+'/5':'—'}</strong></span></div>${o.remarks?`<p class="option-remarks">${esc(o.remarks)}</p>`:''}</div>`
    }).join('')
...
function optionFormHtml(no){ const ratingOptions='<option value="">—</option>'+[1,2,3,4,5].map(n=>`<option value="${n}">${n}</option>`).join(''); return `<section class="option-editor"><div class="option-editor-head"><h3>Option ${no}</h3><label class="choice-radio"><input type="radio" name="selectedPurchaseOption" value="${no}"> Mark as my choice</label></div><div class="form-grid option-fields"><div class="field"><label>Brand</label><input id="pBrand${no}" placeholder="e.g. Faber" /></div><div class="field"><label>Model</label><input id="pModel${no}" placeholder="Model / variant" /></div><div class="field"><label>Price (₹)</label><input id="pPrice${no}" type="number" min="0" step="0.01" placeholder="0" /></div><div class="field"><label>Shop / website</label><input id="pSource${no}" placeholder="Store name or website" /></div><div class="field"><label>Source type</label><select id="pSourceType${no}"><option value="">—</option><option>Shop</option><option>Online</option><option>Other</option></select></div><div class="field"><label>Warranty / validity</label><input id="pWarranty${no}" placeholder="e.g. 5 years motor" /></div><div class="field"><label>Service</label><select id="pService${no}">${ratingOptions}</select></div><div class="field"><label>Quality</label><select id="pQuality${no}">${ratingOptions}</select></div><div class="field"><label>Durability</label><select id="pDurability${no}">${ratingOptions}</select></div><div class="field"><label>Value for money</label><select id="pValue${no}">${ratingOptions}</select></div><div class="field full"><label>Remarks</label><textarea id="pRemarks${no}" rows="2" placeholder="Anything important about this option"></textarea></div></div></section>` }
function openPurchase(id){ const item=state.purchases.find(x=>x.id===id); $('#purchaseForm').reset(); $('#purchaseId').value=item?.id||''; $('#purchaseModalTitle').textContent=item?'Edit purchase comparison':'Add item to compare'; show($('#deletePurchaseBtn'),!!item); $('#purchaseItemName').value=item?.item_name||''; $('#purchaseCategory').value=item?.category||''; $('#purchaseBudget').value=item?.target_budget??''; $('#purchaseDecisionStatus').value=item?.decision_status||'Researching'; $('#purchaseDueDate').value=item?.due_date||''; $('#purchaseCompletedDate').value=item?.completed_at||''; $('#purchaseGeneralRemarks').value=item?.general_remarks||''; const opts=item?purchaseOptionsFor(item.id):[]; [1,2,3].forEach(no=>{ const o=opts.find(x=>x.option_no===no); $('#pBrand'+no).value=o?.brand||''; $('#pModel'+no).value=o?.model||''; $('#pPrice'+no).value=o?.price??''; $('#pSource'+no).value=o?.source_name||''; $('#pSourceType'+no).value=o?.source_type||''; $('#pWarranty'+no).value=o?.warranty||''; $('#pService'+no).value=o?.service_rating||''; $('#pQuality'+no).value=o?.quality_rating||''; $('#pDurability'+no).value=o?.durability_rating||''; $('#pValue'+no).value=o?.value_rating||''; $('#pRemarks'+no).value=o?.remarks||''; const r=document.querySelector(`input[name="selectedPurchaseOption"][value="${no}"]`); if(r)r.checked=!!o?.is_selected }); $('#purchaseModal').classList.remove('hidden'); $('#purchaseModal').setAttribute('aria-hidden','false') }
function closePurchase(){ $('#purchaseModal').classList.add('hidden'); $('#purchaseModal').setAttribute('aria-hidden','true') }
function readOption(no,itemId){ const brand=$('#pBrand'+no).value.trim(),model=$('#pModel'+no).value.trim(),price=$('#pPrice'+no).value,source=$('#pSource'+no).value.trim(),remarks=$('#pRemarks'+no).value.trim(); const has=brand||model||price||source||remarks; if(!has)return null; const selected=document.querySelector('input[name="selectedPurchaseOption"]:checked')?.value==String(no); return {item_id:itemId,option_no:no,brand:brand||null,model:model||null,price:price?Number(price):null,source_name:source||null,source_type:$('#pSourceType'+no).value||null,warranty:$('#pWarranty'+no).value.trim()||null,service_rating:$('#pService'+no).value?Number($('#pService'+no).value):null,quality_rating:$('#pQuality'+no).value?Number($('#pQuality'+no).value):null,durability_rating:$('#pDurability'+no).value?Number($('#pDurability'+no).value):null,value_rating:$('#pValue'+no).value?Number($('#pValue'+no).value):null,remarks:remarks||null,is_selected:selected} }
async function savePurchase(e){ e.preventDefault(); const id=$('#purchaseId').value; const payload={project_id:state.project.id,item_name:$('#purchaseItemName').value.trim(),category:$('#purchaseCategory').value.trim()||null,target_budget:$('#purchaseBudget').value?Number($('#purchaseBudget').value):null,decision_status:$('#purchaseDecisionStatus').value,due_date:$('#purchaseDueDate').value||null,completed_at:$('#purchaseCompletedDate').value||($('#purchaseDecisionStatus').value==='Purchased'?new Date().toISOString().slice(0,10):null),general_remarks:$('#purchaseGeneralRemarks').value.trim()||null,updated_by:state.user.id}; let itemId=id; if(id){ const {error}=await supabase.from('purchase_items').update(payload).eq('id',id); if(error)return toast(error.message) } else { const {data,error}=await supabase.from('purchase_items').insert({...payload,created_by:state.user.id}).select().single(); if(error)return toast(error.message); itemId=data.id }
  const existing=purchaseOptionsFor(itemId); for(const no of [1,2,3]){ const o=readOption(no,itemId),old=existing.find(x=>x.option_no===no); if(o){ const {error}=old?await supabase.from('purchase_options').update(o).eq('id',old.id):await supabase.from('purchase_options').insert(o); if(error)return toast(error.message) } else if(old){ const {error}=await supabase.from('purchase_options').delete().eq('id',old.id); if(error)return toast(error.message) } }
...
$('#purchaseForm').addEventListener('submit',savePurchase); $('#deletePurchaseBtn').addEventListener('click',deletePurchase); $('#exportPurchasesBtn').addEventListener('click',exportPurchases); $('#printPurchasesBtn').addEventListener('click',()=>printPage('purchase')); ['purchaseSearch','purchaseStatusFilter','purchaseTimingFilter'].forEach(id=>$('#'+id).addEventListener(id==='purchaseSearch'?'input':'change',renderPurchaseCompare)); $$('[data-close-purchase]').forEach(b=>b.addEventListener('click',closePurchase))
